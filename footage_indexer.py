"""
Индексация банка футажей: описание видео + эмбеддинги + поиск по смыслу.

Логика в два шага (независимо от остального пайплайна, как qa_agent.py):
  1. Генеративная модель (Gemini) смотрит видео и выдаёт короткое
     текстовое описание содержимого (сцена, объекты, настроение, теги).
  2. Embedding-модель (Gemini Embedding 2) превращает это описание
     в вектор чисел.

Хранение метаданных: НЕ в одном общем файле, а по одному JSON-файлу на
каждое видео, все они складываются в отдельную подпапку `_metadata` внутри
папки с футажами (или в указанную вами папку). Имя файла метаданных совпадает
с именем видео + `.json`. Так проще смотреть/редактировать/удалять метаданные
по отдельности, и не нужно грузить один большой файл целиком.

Поиск: текстовый запрос тоже превращается в вектор той же embedding-моделью,
дальше читаются ВСЕ файлы метаданных из папки и берутся футажи с ближайшими
векторами (косинусное сходство).

Почему эмбеддится ОПИСАНИЕ, а не сырое видео напрямую:
  - дешевле и быстрее (меньше токенов на вызов);
  - нет ограничения по длительности видео на вызов эмбеддинга (~120 сек);
  - описание уже содержит смысл, которого достаточно для поиска по мемам/футажам.
Если понадобится более точный видео-эмбеддинг — можно расширить функцию
_embed_text(), передав видео напрямую вместо текста.
"""
