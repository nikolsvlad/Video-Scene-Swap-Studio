"""
QA-агент: проверяет готовое видео через Gemini.

Оценивает:
  - остался ли персонаж узнаваемым
  - соответствует ли фон/сцена ожидаемому описанию
  - общее качество по шкале 1-10
  - конкретные проблемы и рекомендации по улучшению

Используется отдельно от основного пайплайна обработки видео (gui_app.py).
"""
